...
# Config delivery: prefer OPENCLAW_CONFIG_B64 (embedded), fall back to CONFIG_URL (network fetch)
CONFIG_JSON=""

if [ -n "$OPENCLAW_CONFIG_B64" ]; then
  echo "[entrypoint] Decoding config from OPENCLAW_CONFIG_B64 env var"
  CONFIG_JSON=$(echo "$OPENCLAW_CONFIG_B64" | base64 -d 2>/dev/null || echo "")
  if [ -n "$CONFIG_JSON" ] && echo "$CONFIG_JSON" | jq . > /dev/null 2>&1; then
    echo "[entrypoint] Config decoded successfully from env var"
  else
    echo "[entrypoint] WARNING: Failed to decode OPENCLAW_CONFIG_B64, trying CONFIG_URL..."
    CONFIG_JSON=""
  fi
fi

if [ -z "$CONFIG_JSON" ] && [ -n "$CONFIG_URL" ]; then
  ...
  if [ -n "$CONFIG_JSON" ] && echo "$CONFIG_JSON" | jq . > /dev/null 2>&1; then
    echo "[entrypoint] Config fetched successfully from URL"
  else
    echo "[entrypoint] WARNING: Failed to fetch or parse config from URL"
    CONFIG_JSON=""
  fi
fi

# Process config if we got it from either source
if [ -n "$CONFIG_JSON" ]; then
  ...
  echo "[entrypoint] No config available, using baked-in defaults"
fi
